...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 25
    answer = largest_number(([2, 6, 8], [-10, 25], [7]))
    print('Expected and actual are:', expected, answer)
        ...
        if seq_seq[k] != []:
            ...
            for j in range(len(seq_seq[k])):
                if seq_seq[k][j] > largest:
                    largest = seq_seq[k][j]
            seq_largest = seq_largest + [largest]
    for i in range(len(seq_largest)):
        if seq_largest[i] > largest:
            largest = seq_largest[i]
...
def run_test_largest_negative_number():
    ...
    # Test 2:
    expected = -1111111111111111
    answer = largest_negative_number(([], [-1111111111111111], [20]))
    print('Expected and actual are:', expected, answer)

    # Test 3:
    expected = -1
    answer = largest_negative_number(([-9, 21, 35, -4], [0, 0, 3], [4, -1, 7, -6]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        if seq_seq[k] != []:
            for j in range(len(seq_seq[k])):
                if seq_seq[k][j] < 0:
                    seq_negs = seq_negs + [seq_seq[k][j]]
    if seq_negs != []:
        largest_neg = seq_negs[0]
        for i in range(len(seq_negs)):
            if seq_negs[i] > largest_neg:
                largest_neg = seq_negs[i]
